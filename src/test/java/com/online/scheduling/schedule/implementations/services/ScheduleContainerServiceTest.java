...
class ScheduleContainerServiceTest {
    ...
    void delete_try_deleting_stuff_that_is_involved_in_another_stuff(){
        ...
        schedule1.setScheduleContainer(container1);
        var container2 = new ScheduleContainer(container1);
        container2.setId(3L);
        ...
        schedule2.setScheduleContainer(container2);

        given(scheduleContainerRepository.getPlannedStuffHolders(ids.get(0))).willReturn(List.of(schedule1));
        given(scheduleContainerRepository.findById(2L)).willReturn(Optional.of(container1));
        //when
        var result = scheduleContainerService.delete(ids);
        //then
        assertTrue(result.get(ids.get(0)).contains(schedule1));
        assertFalse(result.get(ids.get(0)).contains(schedule2));
        verify(scheduleContainerRepository, times(0)).deleteAllById(any());
    }
