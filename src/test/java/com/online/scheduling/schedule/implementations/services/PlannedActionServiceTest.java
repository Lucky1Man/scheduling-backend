...
class PlannedActionServiceTest {
    ...
    void delete_try_deleting_stuff_that_is_involved_in_another_stuff(){
        ...
        var action2 = new PlannedAction(action1);
        action2.setId(3L);
        action2.setName("different");
        var day2 = new PlannedDay();
        day2.setId(2L);
        day2.setName("some name 2");
        day2.setUserAccount(userAccount);
        day2.setPlannedActions(Lists.newArrayList(action1));

        given(actionRepository.getPlannedStuffHolders(ids.get(0))).willReturn(List.of(day1));
        given(actionRepository.findById(2L)).willReturn(Optional.of(action1));
        //when
        var result = actionService.delete(ids);
        //then
        assertTrue(result.get(ids.get(0)).contains(day1));
        assertFalse(result.get(ids.get(0)).contains(day2));
        verify(actionRepository, times(0)).deleteAllById(any());
    }
