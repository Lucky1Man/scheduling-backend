...
import com.online.scheduling.exceptions.ValidationExceptionGivenBadTime;
...
public class PATimeBordersValidatorComponentImpl implements IPlannedStuffValidatorComponent<PlannedAction> {
    ...
    public boolean isValid(PlannedAction validated) {//TODO make it better
        ...
        if(startsAt == null)
            throw new ValidationExceptionGivenBadTime(String.format(
                    "Planned action {%s} do not have Starts at value", validated));
        LocalTime endsAt = validated.getEndsAt();
        if(endsAt == null)
            throw new ValidationExceptionGivenBadTime(String.format(
                    "Planned action {%s} do not have Ends at value", validated));
