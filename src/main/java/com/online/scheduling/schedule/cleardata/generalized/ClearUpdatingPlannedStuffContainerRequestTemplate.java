...
public abstract class ClearUpdatingPlannedStuffContainerRequestTemplate <T extends IPlannedStuff<T>> {
    ...
    private void init(){
        ...
        plannedStuffContainers = getContainersToUpdateIgnoringBlankObjects(plannedStuffContainers);
    ...
    private List<T> getContainersToUpdateIgnoringBlankObjects(List<T> containers){
        List<T> result = new ArrayList<>();
        for(var cont : containers ){
            //planned stuff is blank if id != not null and the rest of the fields == null
            if(!cont.isBlank())
                result.add(cont);
        }
        return result;
